...
def evaluate():
	...
	malware_val_dataloader = DataLoader(malware_val, batch_size= batch_size, shuffle=False)
	original_samples = []
	generated_samples = []
	...
	for original_sample,generated_sample in zip(original_samples, generated_samples):
		final_generated_samples.append(['0' if i < 0.5 else '1' for i in generated_sample])
...
def train():
	...
	malware_train_dataloader = DataLoader(malware_train, batch_size= batch_size, shuffle=True)
	malware_val_dataloader = DataLoader(malware_val, batch_size= batch_size, shuffle=False)
	...
	for epoch in range(epochs):
		for i, datapoint in enumerate(malware_train_dataloader):
			...
			valid_centroids = cluster_similar_points(kmeans, z, benign_train).type(torch.FloatTensor)
